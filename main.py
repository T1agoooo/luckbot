...
Fixed global scale used for all chance comparisons
BASE_SCALE = 100_000_000
...
# Use a fixed base roll (1..BASE_SCALE) and convert to an effective roll
# Higher luck_multiplier should make it easier to hit rarer entries,
# so we divide the random base roll by luck_multiplier.
base_roll = random.randint(1, BASE_SCALE)
effective_roll = max(1, int(base_roll / max(1.0, luck_multiplier)))

# Materials: use the same BASE_SCALE and apply drop_multiplier to chances
dropped_materials = {}
for mat_name, mat_data in MATERIALS.items():
adjusted_chance = int(mat_data["chance"] * drop_multiplier)
mat_roll = random.randint(1, BASE_SCALE)
if mat_roll <= adjusted_chance:
inv = user["inventory"]
inv[mat_name] = inv.get(mat_name, 0) + 1
dropped_materials[mat_name] = mat_data["emoji"]

# Dice drops: same fixed scale
got_dice = None
dice_drop_chances = {
"Lucky Dice": 2_000_000,
"Golden Lucky Dice": 200_000,
"Diamond Lucky Dice": 100_000,
"Cosmic Lucky Dice": 10_000,
}
...
# ROLE selection using cumulative ROLE_CHANCES on BASE_SCALE with effective_roll
won_role = None
cumulative = 0
